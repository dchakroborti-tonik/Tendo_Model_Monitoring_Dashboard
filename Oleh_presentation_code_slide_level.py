...
dt_prod_api_oop["ee_onboarding_date"] = pd.to_datetime(
    dt_prod_api_oop["ee_onboarding_date"]
).dt.normalize()
dt_prod_api_oop["onb_rd_diff"] = abs(
    dt_prod_api_oop["run_date"] - dt_prod_api_oop["ee_onboarding_date"]
).dt.days
...
if "ee_onboarding_date_x" in dt_prod_batch_oop.columns:
    dt_prod_batch_oop.rename(
        columns={"ee_onboarding_date_x": "ee_onboarding_date"}, inplace=True
    )

dt_prod_batch_oop["ee_onboarding_date"] = pd.to_datetime(
    dt_prod_batch_oop["ee_onboarding_date"]
).dt.normalize()
...
dt_prod_batch_oop["onb_rd_diff"] = abs(
    dt_prod_batch_oop["run_date"] - dt_prod_batch_oop["ee_onboarding_date"]
).dt.days
dt_prod_batch_oop["osbal_as_of_oop_eligible_date_log"] = np.log1p(
    dt_prod_batch_oop["osbal_as_of_oop_eligible_date"]
)

oop_existing_calc = dt_prod_batch_oop.dropna(
...
def build_attrition_derived(df, date_col):
    ...
    months_diff = (
        df["ee_resignation_date_correct"].dt.year - df[date_col].dt.year
    ) * 12 + (df["ee_resignation_date_correct"].dt.month - df[date_col].dt.month)
    df["time_to_attrition"] = np.where(
        df["ee_resignation_date_correct"].isna(), np.nan, months_diff
    )
...
attr_prod_api = dt_prod_api.merge(dt_res, how="left", on="ee_customer_id").merge(
    dt_bs_attr[
        [
            "ee_customer_id",
            "calc_date_ym",
...
attr_prod_api_calc["attrition_risk_segment_prod"] = attr_prod_api_calc[
    "attrition_score_prod"
].replace(ATTRITION_SCORE_MAP)

# ── Attrition New Users (backscored) ──────────────────────────────────────
attr_bs_new = dt_bs_attr.merge(dt_res, how="left", on="ee_customer_id")
...
attr_prod_batch = dt_prod_batch.merge(dt_res, how="left", on="ee_customer_id").merge(
    dt_bs_attr[
        [
            "ee_customer_id",
            "calc_date_ym",
...
attr_prod_batch_calc["attrition_risk_segment_prod"] = attr_prod_batch_calc[
    "attrition_score_prod"
].replace(ATTRITION_SCORE_MAP)

# ── Attrition Existing Users (backscored) ─────────────────────────────────
attr_bs_existing_calc = (
    dt_bs_attr.merge(dt_res, how="left", on="ee_customer_id")
    .query("is_new_customer_flag_3m == 0")
    .copy()
)
attr_bs_existing_calc = build_attrition_derived(
    attr_bs_existing_calc, "calc_date_correct"
)
attr_bs_existing_calc["score_attr_corrected"] = attr_bs_existing_calc[
    "score_attr"
].replace(np.inf, 15)
...
gini_oop_ex_prod = calculate_gini(
    ...
    Model="OOP Repay", ModelVersion="v1.0 Prod", UserType="Existing users (>= 3MOB)"
)
...
gini_attr_ex_prod = calculate_gini(
    ...
    Model="Attrition", ModelVersion="v1.0 Prod", UserType="Existing users (>= 3MOB)"
)
...
page2 = pd.concat(
    ...
    ignore_index=True,
).rename(columns={"Gini": "MetricValue"})

# Add dev numbers from the slide (these are the fixed training-time numbers)
DEV_NUMBERS = [
    {
        "UserType": "New users (< 3MOB)",
        "Model": "OOP Repay",
        "ModelVersion": "v1.0 Prod",
        "Period": "Dev",
        "MetricValue": 0.30,
    },
    {
        "UserType": "Existing users (>= 3MOB)",
        "Model": "OOP Repay",
        "ModelVersion": "v1.0 Prod",
        "Period": "Dev",
        "MetricValue": 0.32,
...
def build_oop_rank_order(
    df, risk_group_col, snapshot_filter_expr, model_version_label, date_label
):
...
def build_attrition_rank_order(
    df,
    segment_col,
    filter_expr,
    model_label,
    period_label,
    date_col="calc_date_correct",
):
...
dt_utilization["UtilizationRate_Pct"] = (dt_utilization["UtilizationRate"] * 100).round(
    1
)
